	...
	for k, v in pairs(Config.Locations) do
		if v.garage then
			...
			exports['qb-target']:AddBoxZone("JobGarage: "..k, vector3(out.x, out.y, out.z-1.03), 0.8, 0.5, { name="JobGarage: "..k, heading = out.w+180.0, debugPoly=Config.Debug, minZ=out.z-1.05, maxZ=out.z+0.80 },
				{ options = { { event = "jim-jobgarage:client:Garage:Menu", icon = "fas fa-clipboard", label = " VIEW", job = v.job, coords = v.garage.spawn, list = v.garage.list, prop = Parking[#Parking] }, },
				distance = 2.0 })
		end
	end
end)
...
RegisterNetEvent('jim-jobgarage:client:Garage:Menu', function(data)
	...
	local vehicleMenu = { { icon = "fas fa-car-tunnel", header = "GARAGE", isMenuHeader = true } }
	vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-circle-xmark", header = "", txt = "EXIT", params = { event = "jim-jobgarage:client:Menu:Close" } }
		...
		for k, v in pairs(data.list) do
			...
			if showButton == true then
				...
				local classtable = {
					...
					[18] = "fas fa-truck-front", -- Emergency
				}
	...
	showButton = nil
	if #vehicleMenu <= 2 then triggerNotify(nil, "Unavailable") return end
...
RegisterNetEvent("jim-jobgarage:client:Garage:Blip", function(data)
	triggerNotify(nil, "Vehicle Marked!")
